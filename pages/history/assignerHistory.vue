<template>
	<view>
		<view class="assignerHistory">
			<view class="assign_item"  v-for="item in orderList">
				<view class="number">
					<text>订单编号：{{item.id}}</text>
					<text class="done" v-if="item.finishTime!==''">已付款</text>
					<text class="notdone" v-else>未付款</text>
					<view style="position: absolute;right: 20rpx;">
						<image src="../../static/rightArrow.png" 
						style="width: 30rpx;height: 30rpx;"
						@click="goToAssignOrderDetail(item)"></image>
					</view>
				</view>
...
<script>
	export default{
		...
		methods:{
			goToAssignOrderDetail(item){
				console.log("派单者订单详情" + JSON.stringify(item));
				uni.navigateTo({
					url:"../assign/assignOrderDetail?item=" + JSON.stringify(item),
				})
			},
			async getAllAssignerHistory(id){
				// var that = this;
		...
		onLoad() {
			...
			this.getAllAssignerHistory(uni.getStorageSync("stu_id"));
